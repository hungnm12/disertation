...
key, value in document.items() if
    ...
    for key, value in document.items():
        if key not in ['_id', '_class'] or (key == '_id' and not SKIP_ID_FIELD) or (
                key == '_class' and not SKIP_CLASS_FIELD):
            if isinstance(value, dict):
                structure.update(process_nested_document(value, camel_to_snake(key)))
            else:
                structure[camel_to_snake(key)] = type_to_mysql(camel_to_snake(key), type(value).__name__,
                                                               max_lengths.get(camel_to_snake(key)))
...
def insert_into_mysql(mysql_cursor, collection_name, document):
    collection_name = camel_to_snake(collection_name)
    document = {camel_to_snake(key): value for key, value in document.items() if
                key not in ['_id', '_class'] or (key == '_id' and not SKIP_ID_FIELD) or (
                        key == '_class' and not SKIP_CLASS_FIELD)}
    ...
    while True:
        try:
            ...
            if 'Unknown column' in str(e):
                ...
                varchar_size = min(
                    max(DEFAULT_VARCHAR_SIZE, (value_length // DEFAULT_VARCHAR_SIZE + 1) * DEFAULT_VARCHAR_SIZE),
                    MAX_VARCHAR_LENGTH)
...
if __name__ == '__main__':
    ...
    # Consume messages and update configurations
    kafka_consumer.consume_and_update_configs()

    # Once configurations are received, start the migration
    start_migration()